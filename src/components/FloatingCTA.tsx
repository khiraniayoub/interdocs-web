...
const TRANSLATIONS: Record<string, any> = {
  ...
  ru: { call: "Позвонить", whatsapp: "WhatsApp", whatsappMobile: "WhatsApp", title: "Быстрый вызов врача", q1: "1. Где вы находитесь?", p1: "Название отеля или адрес...", q2: "2. Какие у вас симптомы?", p2: "Кратко опишите проблему...", btn: "Перейти в WhatsApp", msg: "Здравствуйте, мне нужен врач.\n\n📍 Местоположение: {location}\n🤒 Симптомы: {symptoms}" },
  nl: { call: "Bel nu", whatsapp: "WhatsApp nu", whatsappMobile: "WhatsApp", title: "Snelle doktersaanvraag", q1: "1. Waar verblijft u?", p1: "Hotelnaam of adres...", q2: "2. Wat zijn uw symptomen?", p2: "Beschrijf kort uw klachten...", btn: "Verder naar WhatsApp", msg: "Hallo, ik heb een dokter nodig.\n\n📍 Locatie: {location}\n🤒 Symptomen: {symptoms}" }
};
...
export default function FloatingCTA({ locale: propLocale = "en" }: { locale?: string }) {
  ...
  const whatsappIcon = (
    <svg className="w-5 h-5 lg:w-6 lg:h-6 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  return (
    ...
    >
        ...
        <div className="absolute bottom-full right-4 left-4 lg:left-auto lg:right-0 mb-4 lg:w-[340px] bg-white rounded-2xl shadow-2xl border border-slate-100 p-5 transform origin-bottom transition-all animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <p className="font-700 text-slate-800">{t.title}</p>
            </div>
            <button 
      ...
      {/* Mobile: Full-width bar with phone visible */}
      <div className="flex lg:hidden shadow-[0_-10px_40px_rgba(0,0,0,0.15)] relative z-10">
        <a
          href={PHONE_URL}
          id="floating-call-btn"
          className="flex-1 flex items-center justify-center gap-2.5 py-3 px-2 bg-slate-900 text-white active:bg-slate-700 transition-colors"
        ...
        >
          ...
          <div className="flex flex-col text-left">
            <span className="text-[10px] uppercase font-bold text-slate-400 leading-none">{t.call}</span>
            <span className="text-xs sm:text-sm font-extrabold tracking-tight text-white leading-tight mt-0.5">{PHONE_NUMBER}</span>
          </div>
        </a>
        <button
          onClick={handleWhatsAppClick}
          id="floating-whatsapp-btn"
          className="flex-1 flex items-center justify-center gap-2 py-3 px-2 bg-[#25D366] text-white active:bg-[#1da851] transition-colors"
          aria-label={t.whatsappMobile}
        >
          {whatsappIcon}
          <div className="flex flex-col text-left">
            <span className="text-[10px] uppercase font-bold text-emerald-100 leading-none">24/7 Chat</span>
            <span className="text-xs sm:text-sm font-extrabold tracking-tight text-white leading-tight mt-0.5">{t.whatsappMobile}</span>
          </div>
        </button>
      </div>

      {/* Desktop: Floating buttons with full phone visible */}
      <div className="hidden lg:flex flex-col gap-3 relative z-10">
        <button
          onClick={handleWhatsAppClick}
          id="floating-whatsapp-desktop"
          className="flex items-center justify-start gap-3 px-5 py-3.5 bg-[#25D366] text-white rounded-2xl shadow-xl hover:bg-[#1da851] hover:shadow-2xl hover:-translate-y-0.5 transition-all duration-200 min-w-[230px]"
          aria-label={t.whatsapp}
        >
          {whatsappIcon}
          <div className="flex flex-col text-left">
            <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-100 leading-tight">Online 24/7</span>
        ...
        <a
          ...
          className="flex items-center justify-start gap-3 px-5 py-3.5 bg-[#0A6EBD] text-white rounded-2xl shadow-xl hover:bg-[#085a99] hover:shadow-2xl hover:-translate-y-0.5 transition-all duration-200 min-w-[230px]"
          aria-label={`${t.call} ${PHONE_NUMBER}`}
        >
          <svg className="w-5 h-5 text-sky-200 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
          </svg>
          <div className="flex flex-col text-left">
            <span className="text-[10px] font-bold uppercase tracking-wider text-sky-200 leading-tight">{t.call}</span>
            <span className="text-sm font-extrabold tracking-wide leading-tight">{PHONE_NUMBER}</span>
          </div>
        </a>
      </div>
    </div>
